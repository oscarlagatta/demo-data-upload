...
const CustomNodeUsWires = ({
  ...
  const isLoading = !splunkData
  const isError = false // Error handling now managed by parent
  const isFetching = false // Fetching state now managed by parent
  ...
  const trendColorMapping = useMemo(() => {
    ...
    // Flatten splunk data from all nodes
